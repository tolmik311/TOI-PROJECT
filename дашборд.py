...
async def predict(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Введите ваш возраст (20-80 лет):")
    return AGE
...
async def age(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if not 20 <= age <= 80:
            await update.message.reply_text("Пожалуйста, введите возраст от 20 до 80:")
            return AGE
...
async def gender(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if gender not in ['Male', 'Female']:
        await update.message.reply_text(
            "Пожалуйста, выберите пол из предложенных:",
            reply_markup=ReplyKeyboardMarkup(gender_keyboard, one_time_keyboard=True, resize_keyboard=True)
        )
        return GENDER
    context.user_data['Gender'] = gender
    await update.message.reply_text("Уровень холестерина (120-240 мг/дл):", reply_markup=ReplyKeyboardRemove())
    return CHOLESTEROL

async def cholesterol(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        cholesterol = int(update.message.text)
        if not 120 <= cholesterol <= 240:
            await update.message.reply_text("Пожалуйста, введите значение от 120 до 240:")
            return CHOLESTEROL
        context.user_data['Cholesterol'] = cholesterol
        await update.message.reply_text("Артериальное давление (90-140 мм рт.ст.):")
...
async def blood_pressure(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if not 90 <= bp <= 140:
            await update.message.reply_text("Пожалуйста, введите значение от 90 до 140:")
            return BLOOD_PRESSURE
        context.user_data['BloodPressure'] = bp
        await update.message.reply_text("Пульс в покое (50-90 уд/мин):")
        return HEART_RATE
    except ValueError:
        await update.message.reply_text("Пожалуйста, введите число:")
        return BLOOD_PRESSURE
...
async def heart_rate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if not 50 <= hr <= 90:
            await update.message.reply_text("Пожалуйста, введите значение от 50 до 90:")
            return HEART_RATE
...
async def alcohol_consumption(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("Тип питания:", reply_markup=reply_markup)
        return DIET
    except ValueError:
        await update.message.reply_text("Пожалуйста, введите число от 0 до 10:")
        return ALCOHOL_CONSUMPTION
...
async def diet(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if diet not in ['Healthy', 'Moderate', 'Unhealthy']:
        await update.message.reply_text(
            "Пожалуйста, выберите тип питания:",
            reply_markup=ReplyKeyboardMarkup(diet_keyboard, one_time_keyboard=True, resize_keyboard=True)
        )
        return DIET
...
async def calculate_result(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        # Формирование ответа
        if risk_percent < 15:
            message = f"✅ Очень низкий риск: {risk_percent:.1f}%\n\nВаши показатели в отличном состоянии!"
        elif risk_percent < 30:
            message = f"🟢 Низкий риск: {risk_percent:.1f}%\n\nПродолжайте вести здоровый образ жизни."
        elif risk_percent < 50:
            message = f"🟡 Умеренный риск: {risk_percent:.1f}%\n\nРекомендуется профилактический осмотр."
        else:
            message = f"🔴 Высокий риск: {risk_percent:.1f}%\n\nРекомендуем обратиться к кардиологу."
